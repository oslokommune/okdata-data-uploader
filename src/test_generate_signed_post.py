...
def api_gateway_event():
    ...
    """
    def _event(principalId="test-dataplatform", body=json.dumps({"distributionId": "foo"})):
        return {
        "body": body,
        "httpMethod": "POST",
...
def test_handler_404_when_not_authenticated(api_gateway_event):
    ...
    assert json.loads(ret["body"])["message"] == "Forbidden. Only the test user can do this"
...
def test_handler_invalid_json(api_gateway_event):
    ...
    ret = handler(event, None)
    assert ret["statusCode"] == 400
    assert json.loads(ret["body"])["message"] == "JSON document does not conform to the given schema"
